...
Each page uses existing scoped search, filters, ordering and pagination. A one-hour signed manifest binds the current actor/site/model, field names, exact page identities, version/snapshot digests and a hash of the query. It contains no field values, labels or raw filter/search text. The page is bounded by `ListPerPage` (maximum 1000) and the signer's payload bound. A changed page, expired token, identity substitution, duplicated management data or ambiguous action/save submission is rejected; reload before editing again. Unknown extra model fields and readonly row values are ignored, never assigned.
