...
func Generate(projectDir string) error {
	return generateModels(projectDir, false)
}

// CheckGenerated compares model references and registrations with their current
// Go declarations without writing source or evaluating application code. A
// development reload must pass this check before replacing a healthy process;
// authors explicitly run generate to repair stale or missing descriptors.
func CheckGenerated(projectDir string) error {
	return generateModels(projectDir, true)
}

func generateModels(projectDir string, check bool) error {
	...
	for _, entry := range entries {
		...
		if err = generateApp(root, entry.Name(), check); err != nil {
			return fmt.Errorf("generate app %s: %w", entry.Name(), err)
		}
	}
	return nil
}
func generateApp(root *os.Root, label string, check bool) error {
	...
	if check {
		// Do not follow a descriptor link or treat a directory/device as output.
		info, err := root.Lstat(target)
		if err != nil || !info.Mode().IsRegular() {
			return errors.New("generated model descriptors are missing or invalid; run generate")
		}
		...
		if !bytes.Equal(existing, generated) {
			return errors.New("generated model descriptors are stale; run generate")
		}
		return nil
	}
	existing, err := root.ReadFile(target)
