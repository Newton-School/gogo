...
[Workflow result boundaries](group_result.md) freeze each operation's client/identity, validate and detach bounded provider graphs before authorization, reject malformed observations without partial disclosure, and recheck the workflow after authorized child-payload recovery. Missing successful retained output is an expiry error, never fabricated success. A failed cancellation command can follow an applied request; reconcile through a fresh authorized snapshot.
