...
For opt-in `manage.go openapi` emission of the same document, register the separate [OpenAPI management command](../management/openapi.md).

This is the read-only OpenAPI slice. Input and mutation schemas, arbitrary route metadata, typed clients and a browsable documentation UI remain separate work. It is not an exhaustive Django/DRF compatibility or release claim. OpenAPI semantics follow the [3.1.1 specification](https://spec.openapis.org/oas/v3.1.1.html); inline schemas inherit the document dialect without incorrectly adding standalone `$schema` declarations inside subschemas.
