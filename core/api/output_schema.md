...
This is the serializer-output foundation. For handler-bound read routes and explicit authentication metadata, see [read-route OpenAPI documents](openapi.md), with an opt-in [management command](../management/openapi.md) for emission. Input/write schemas, complete route coverage, typed client generation and browsable documentation remain separate work. Neither API is a Django/DRF compatibility or release claim.
