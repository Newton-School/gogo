...
func Open(ctx context.Context, config Config) (*Backend, error) {
	...
	backend := &Backend{pool: pool, alias: config.Alias, capabilities: db.Capabilities{"returning": true, "transactions": true, "savepoints": true, "row_locks": true, "json": true, "arrays": true, "ranges": true, "window": true, "regex": true, "distinct_on": true, "transactional_ddl": true, "concurrent_indexes": true}}
